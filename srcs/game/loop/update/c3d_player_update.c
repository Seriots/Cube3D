...
/*   Updated: 2022/09/29 11:27:10 by lgiband          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	update_movement(t_game *game)
{
	...
	if (game->map.map[(int)floor(game->player.pos.y / CASE_SIZE)]
		[(int)floor((game->player.pos.x + mov_x + game->settings.fov
					* pure_sign(mov_x)) / CASE_SIZE)] != '1')
		game->player.pos.x += mov_x;
	if (game->map.map[(int)floor((game->player.pos.y + mov_y
					+ game->settings.fov * pure_sign(mov_y)) / CASE_SIZE)]
		[(int)floor(game->player.pos.x / CASE_SIZE)] != '1')
