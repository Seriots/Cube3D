...
/*   Updated: 2022/10/03 17:12:52 by lgiband          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*get_reduce_path(char path[256], t_textinput *textinput)
{
	...
	else if (textinput->size - textinput->start_display < textinput->max_char_display)
		ft_strlcpy(path, &textinput->path[textinput->size - textinput->max_char_display], textinput->max_char_display + 1);
	else
	{
		ft_strlcpy(path, &textinput->path[textinput->start_display], textinput->max_char_display + 1 - 3);
...
int	display_textinput_text(t_game *game, t_textinput *textinput)
{
	...
	mlx_string_put(game->mlx.display, game->mlx.window,
		...
		(WIN_HEIGHT / 2) - (MENU_HEIGHT / 2) + textinput->box.y_text,
		0xAAAAAA, textinput->box.description);
	mlx_string_put(game->mlx.display, game->mlx.window,
		(WIN_WIDTH / 2) - (MENU_WIDTH / 2) + textinput->box.x + 2,
		(WIN_HEIGHT / 2) - (MENU_HEIGHT / 2) + textinput->box.y + (textinput->box.height / 2) + 5,
