...
/*   Updated: 2022/10/04 22:21:47 by lgiband          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	display_rgb_value(t_game *game, t_numinput *numinput,
	char buf[32], int y)
{
...
int	display_numinput_text(t_game *game, t_numinput *numinput)
{
	...
	y = (WIN_HEIGHT / 2) - (MENU_HEIGHT / 2)
		+ numinput->box.y_text + game->menu.scroll_amount;
	if (y - (WIN_HEIGHT / 2 - MENU_HEIGHT / 2) >= 9
		&& y - (WIN_HEIGHT / 2 - MENU_HEIGHT / 2)
		< (MENU_HEIGHT - (15 * MENU_HEIGHT / 100)))
		display_rgb_value(game, numinput, buf, y);
	return (0);
}
