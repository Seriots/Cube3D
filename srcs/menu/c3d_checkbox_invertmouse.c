...
/*   Updated: 2022/10/05 13:37:00 by lgiband          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	invertmouse_release(int button, int x, int y, t_game *game)
{
	t_checkbox	*checkbox;

	if (button == 1)
	{
		checkbox = dict_getelem_number(game->menu.all_objects, 19)->value;
...
int	invertmouse_press(int button, int x, int y, t_game *game)
{
	(void)x;
	(void)y;
	if (button == 1)
		game->fcts.mousereleased_fct = invertmouse_release;
	return (0);
}
...
t_dict	*init_invertmouse_checkbox(t_game *game)
{
	...
	checkbox->box.mouse_press = invertmouse_press;
