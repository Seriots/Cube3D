...
/*   Updated: 2022/10/04 19:52:59 by lgiband          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	scrollbar_mouse_move(int x, int y, t_game *game)
{
	...
	percent_pos = (float)(value->box.y - value->min) / (float)(value->max - value->min);
...
int	scrollbar_press(int button, int x, int y, t_game *game)
{
	...
	if (button == 1)
	{
		value = (dict_getelem_key(game->menu.all_objects, SCROLLBAR)->value);
		value->is_selected = 1;	
		value->click_position = y - (WIN_HEIGHT / 2 - MENU_HEIGHT / 2) - value->box.y;
...
int set_height(t_game *game, int *max_obj)
{
	...
	while (elem)
	{
		if (ft_strcmp(elem->key, BUTTON) != 0 && ((t_collide_box *)elem->value)->y
				+ ((t_collide_box *)elem->value)->height > max)
			max = ((t_collide_box *)elem->value)->y
					+ ((t_collide_box *)elem->value)->height - (MENU_HEIGHT - (15 * MENU_HEIGHT / 100));
		elem = elem->next;
	}
	*max_obj = max;
...
t_dict	*init_scrollbar(t_game *game)
{
	...
	scrollbar->max = (MENU_HEIGHT - (15 * MENU_HEIGHT / 100)) - 7 - scrollbar->box.height;
	scrollbar->min = 7;
	...
	ft_strlcpy(scrollbar->box.font, "-sony-*-*-*-*-*-*-230-*-*-*-*-iso8859-*", 40);
