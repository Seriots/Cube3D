...
int	mapinput_keyrelease(KeySym key, t_game *game)
{
	t_textinput	*textinput;
...
int	mapinput_release(int button, int x, int y, t_game *game)
{
	t_textinput	*textinput;
	...
	if (release_cond_textinput_start(game, textinput, x, y))
	{
		...
		if (textinput->is_selected)
		{
			...
			game->fcts.keypressed_fct = mapinput_keypress;
			game->fcts.keyreleased_fct = mapinput_keyrelease;
...
int	mapinput_press(int button, int x, int y, t_game *game)
{
	...
	(void)y;
	if (button == 1)
		game->fcts.mousereleased_fct = mapinput_release;
	return (0);
}
