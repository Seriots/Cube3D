...
{
	if (x <= box->box.x + 3 * box->box.width / 16 + 1
		|| x >= box->box.x + 13 * box->box.width / 16 - 1
...
int	draw_first_triangle(t_game *game, t_multichoice *box, int x, int y)
{
	...
	max = ((((float)(x - box->box.x) / (box->box.width / 8)))
			* (box->box.height / 2.0f)) + box->box.height / 2.0f;
	min = floor(box->box.height / 2.0f - ((((float)(x - box->box.x)
						/ (box->box.width / 8))) * (box->box.height / 2.0f)));
...
int	draw_second_triangle(t_game *game, t_multichoice *box, int x, int y)
{
	...
	max = (box->box.height / 2.0f)
		- (((float)(x - box->box.x - 7 * (box->box.width / 8))
				/ (box->box.width / 8))) * (box->box.height / 2.0f);
	min = -max;
