...
/*   Updated: 2022/10/28 12:57:37 by lgiband          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_wall_test2_collide(t_game *game, t_map *map, t_vector vec, t_wall *wall)
{
	if (map->map[((int)vec.y + 1) / CASE_SIZE][((int)vec.x) / CASE_SIZE]
			== '1')
		wall->face = 'N';
	...
	if (map->map[((int)vec.y + 1) / CASE_SIZE][((int)vec.x) / CASE_SIZE]
		...
		if (find_door(game, ((int)vec.x) / CASE_SIZE,
				((int)vec.y + 1) / CASE_SIZE)->face == 'N'
				&& find_door(game, ((int)vec.x) / CASE_SIZE, ((int)vec.y + 1) / CASE_SIZE)->state == 0)
			wall->face = 'n';
	if (map->map[((int)vec.y - 1) / CASE_SIZE][((int)vec.x) / CASE_SIZE]
			== '2')
		if (find_door(game, ((int)vec.x) / CASE_SIZE,
...
int	check_wall_test1_collide(t_game *game, t_map *map, t_vector vec, t_wall *wall)
{
	if (map->map[((int)vec.y) / CASE_SIZE][((int)vec.x + 1) / CASE_SIZE]
			== '1')
		wall->face = 'W';
	...
	if (map->map[((int)vec.y) / CASE_SIZE][((int)vec.x + 1) / CASE_SIZE]
		...
		if (find_door(game, ((int)vec.x + 1) / CASE_SIZE,
				((int)vec.y) / CASE_SIZE)->face == 'W'
				&& find_door(game, ((int)vec.x + 1) / CASE_SIZE, ((int)vec.y) / CASE_SIZE)->state == 0)
			wall->face = 'w';
	if (map->map[((int)vec.y) / CASE_SIZE][((int)vec.x - 1) / CASE_SIZE]
			== '2')
		if (find_door(game, ((int)vec.x - 1) / CASE_SIZE,
...
int	check_wall_collide(t_game *game, t_map *map, t_vector vec, t_wall *wall)
{
	(void)game;
	if (vec.x - (int)vec.x == 0 && (int)vec.x % CASE_SIZE == 0)
		check_wall_test1_collide(game, map, vec, wall);
	else
		check_wall_test2_collide(game, map, vec, wall);
	return (wall->face);
}
