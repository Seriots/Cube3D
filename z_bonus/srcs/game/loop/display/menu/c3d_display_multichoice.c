...
/*   Updated: 2022/10/23 16:27:23 by lgiband          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	display_multichoice_text(t_game *game, t_multichoice *multichoice)
{
	...
	mlx_string_put(game->mlx.display, game->mlx.window, multichoice->box.x
		...
		- (FONT_WIDTH
			* ft_strlen(multichoice->all_values[*multichoice->modified_value])) / 2,
		y, 0xAAAAAA, multichoice->all_values[*multichoice->modified_value]);
	return (0);
}
